...
const StdFee = Root.hashgard.StdFee;
const Coin = Root.hashgard.Coin;
...
class StdTx {
    ...
    SetSignature(sig) {
        ...
        let signature = new Root.hashgard.StdSignature({
            pubKey: sig.pub_key,
    ...
    SetPubKey(pubkey) {
        if (Codec.Bech32.isBech32(Config.hashgard.bech32.accPub, pubkey)) {
    ...
    GetData() {
        ...
        if (this.signatures) {
            this.signatures.forEach(function (sig) {
                ...
                signatures.push({
                    pub_key: Amino.MarshalJSON(Config.hashgard.amino.pubKey, publicKey),
                    signature: signature,
                })
            });
        }
