...
class HashgardBuilder extends Builder {
    ...
    buildTx(tx) {
        ...
        let msg;
        switch (req.type) {
            case Config.hashgard.tx.transfer.type: {
                msg = Bank.create(req);
                break;
            }
            ...
            case Config.hashgard.tx.undelegate.type: {
                msg = Stake.createMsgUndelegate(req);
                break;
            }
            case Config.hashgard.tx.beginRedelegate.type: {
                msg = Stake.createMsgBeginRedelegate(req);
                break;
            }
            case Config.hashgard.tx.setWithdrawAddress.type: {
                msg = Distribution.CreateMsgSetWithdrawAddress(req);
                break;
            }
            case Config.hashgard.tx.withdrawDelegatorReward.type: {
                msg = Distribution.CreateMsgWithdrawDelegatorReward(req);
                break;
            }
            case Config.hashgard.tx.withdrawValidatorCommission.type: {
    ...
    sign(data, privateKey) {
        ...
        let signbyte = HashgardKeypair.sign(privateKey, data);
